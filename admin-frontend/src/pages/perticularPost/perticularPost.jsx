...
function PostUI({ post, setPost }) {
  ...
  async function handleDeletePost() {
    ...
    try {
      let response = await deletePost(post.slug, accessToken);

      if (response.status === 401) {
        const newAccessToken = await refreshAccessToken();
        response = await deletePost(post.slug, newAccessToken);
      }
    ...
    <main className={styles.post}>
      <section className={styles.postCard}>
        <div className={styles.postActions}>
          ...
          {user && (user.role === "OWNER" || user.id === post.author.id) && (
            <div className={styles.actionLinks}>
              {error && <p className={styles.error}>{error.message}</p>}
              {deleting && <p className={styles.deleting}>Deleting post...</p>}
              <button
              ...
              <Link to={`/posts/${post.slug}/edit`} className={styles.editLink}>
                Edit Post
              </Link>
            </div>
          )}
